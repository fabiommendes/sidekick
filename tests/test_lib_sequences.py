...
class TestToolzSuite:
    ...
    def test_merge_sorted(self):
        ...
        assert (
            "".join(merge_sorted("cba", "cba", "cba", key=lambda x: -ord(x)))
            == "cccbbbaaa"
        )
    ...
    def test_mapcat(self):
        ...
        assert [4, 5, 6, 7, 8, 9] == list(
            mapcat(partial(map, inc), [[3, 4, 5], [6, 7, 8]])
        )
    ...
    def test_frequencies(self):
        assert frequencies(
            ["cat", "pig", "cat", "eel", "pig", "dog", "dog", "dog"]
    ...
    def test_reduce_by(self):
        ...
        assert reduce_by(
            lambda x: x["state"], lambda acc, x: acc + x["cost"], projects, init=0
        ) == {"CA": 1200000, "IL": 2100000}

        assert reduce_by("state", lambda acc, x: acc + x["cost"], projects, init=0) == {
            "CA": 1200000,
            "IL": 2100000,
        }

    def test_reduce_by_init(self):
        assert reduce_by(iseven, add, [1, 2, 3, 4]) == {True: 2 + 4, False: 1 + 3}
    ...
    def test_pluck(self):
        ...
        assert list(pluck(0, [[0, 1], [2, 3], [4, 5]], default=no_default2)) == [
            0,
            2,
            4,
        ]
    ...
    def test_random_sample(self):
        ...
        assert list(random_sample(1, alist, random_state=2016)) == alist

        mk_rsample = lambda rs=1: list(random_sample(0.1, alist, random_state=rs))
        rsample1 = mk_rsample()
